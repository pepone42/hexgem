...
struct BOARD
{
	...
	int timer;
	int itmer_max;
	int[GEMCOL.MAXGEMCOL*2][4] scores;
	int scores_index;
	int combo;
	int combo_cnt;
	int chain;
...
GEMLIST *glist_append(GEMLIST *gl,GEM *g) {
	...
	for (l=gl;l.next!=NULL;l=l.next) {} // Bug???
		l.next=glt;
	return gl;
}
...
GEMLIST *glist_insert(GEMLIST *gl,GEM *g) {
	...
	for(t=gl;t && g.fall_y<t.g.fall_y;pt=t,t=t.next) {} // Bug???
		if (t) {
...
BOARD *create_board(int type,int w,int h,int maxcol) {
	...
	b.fallbuf=malloc((GEMLIST*).sizeof*w);
	memset(b.fallbuf,0,(GEMLIST*).sizeof*w);
	b.gcount=malloc(int.sizeof*GEMCOL.MAXGEMCOL);
	memset(b.gcount,0,int.sizeof*GEMCOL.MAXGEMCOL);
	b.w=w;
	b.h=h;
	b.cursx=-1;
	b.cursy=-1;
...
int reduce_board(BOARD *b) {
	...
	GEM *g;
	int fall_y;
	int[GEMCOL.MAXGEMCOL][3] scores_helper;
	int count;

	for (i=0;i<GEMCOL.MAXGEMCOL;i++) {
	...
	for (i=0;i<GEMCOL.MAXGEMCOL;i++) {
		if (scores_helper[i][2]!=0) {
			...
			b.scores_index++;
			if (b.scores_index>=GEMCOL.MAXGEMCOL)
				b.scores_index=0;
		}
	}
	if (rc>0) {
